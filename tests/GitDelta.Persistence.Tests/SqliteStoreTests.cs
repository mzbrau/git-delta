...
public sealed class SqliteStoreTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "GitDelta.Tests", Guid.NewGuid().ToString("N"), "cache.db");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "GitDelta.Tests", Guid.NewGuid().ToString("N"), "durable.db");
