...
public sealed class WorkingCopyViewModelHistoryCacheTests
{
    ...
    {
        var repo1 = Path.Combine(Path.GetTempPath(), "gitdelta-tests", Guid.NewGuid().ToString("N"));
        var repo2 = Path.Combine(Path.GetTempPath(), "gitdelta-tests", Guid.NewGuid().ToString("N"));
